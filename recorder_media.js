...
let isCameraHardwareOn = true; 
let isAudioHardwareOn = true; 
...
async function initCameraHardware() {
    try { 
        ...
        isCameraPermissionGranted = true; isMicrophonePermissionGranted = true;
        document.getElementById('camHardwareErrorNotice').classList.add('hidden');
    } catch (e) { 
        isCameraHardwareOn = false; isAudioHardwareOn = false; 
        isCameraPermissionGranted = false; isMicrophonePermissionGranted = false;
...
document.getElementById('btnToggleCam').addEventListener('click', () => { 
    ...
    isCameraHardwareOn = !isCameraHardwareOn; camVideoFeed.style.opacity = isCameraHardwareOn ? '1' : '0'; 
});
...
liveBadge.addEventListener('click', async () => {
    ...
    masterAudioVideoRecorder.onstop = async () => {
        ...
        const compressedSlidesArray = window.globalSlidesDeck.map((slide, sIdx) => {
            let isolatedSourceUrl = null;
            let isolatedThumbnailUrl = null;
            ...
            if (slide.sourceUrl && slide.sourceUrl.includes(',')) {
                ...
                isolatedSourceUrl = slide.sourceUrl;
            }
            ...
            if (slide.thumbnail && slide.thumbnail.includes(',')) {
                ...
                isolatedThumbnailUrl = slide.thumbnail;
            }
            ...
            return {
                ...
                name: slide.name,
                type: slide.type,
                sourceUrl: isolatedSourceUrl,
                thumbnail: isolatedThumbnailUrl,
                annotation: slide.annotation
            };
        });

        const dataJsonPayload = { 
            version: "Shiv Board Pro V5", 
            timestamp: Date.now(), 
            slides: compressedSlidesArray, 
            timeline: window.jsonDrawingTimelineLog 
        };
        
        zip.file("data.json", JSON.stringify(dataJsonPayload));
        
        const compressedArchiveBlob = await zip.generateAsync({
            type: "blob",
            compression: "DEFLATE",
...
window.addEventListener('keyup', (e) => {
    if (e.key === 'Alt') { document.getElementById('shortcutHudOverlay').style.display = 'none'; }
});